...
void Symbol_Table::print_directives(Emitter *e)
{
...
void Symbol_Table::print_contents(int i)
{
  cout<< "The current id is: "<< *table[i].id  <<" environment: " << *table[i].env <<" type: " << *to_string(table[i].type) <<" position: " << table[i].position << endl;
}
